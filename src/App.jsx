...
const HISTORY_KEY = "atlaris.customer-monitor-history.v1";
const ALERT_KEY = "atlaris.monitor-alerts.v1";
const HISTORY_LIMIT = 240;
const ALERT_LIMIT = 100;

const customers = [
  { id: "CUS-001", name: "Sunshine Solution", tier: "Managed", status: "Active", environments: 1, services: 2, website: "https://sunshine-backend.pages.dev/", brandIcon: "☀", brandClass: "sunshine", description: "Primary application plus a dedicated support service managed through Atlaris customer operations.", sites: [
  ...
  { id: "CUS-002", name: "Vancouver RealEstateIQ", tier: "Managed", status: "Active", environments: 1, services: 1, website: "https://realt-state-housing.pages.dev/", brandIcon: "VR", brandClass: "realestate", description: "Vancouver RealEstateIQ managed customer environment.", sites: [
  ...
  { id: "CUS-003", name: "AIport", tier: "Managed", status: "Active", environments: 1, services: 1, website: "https://aiports-backend-final-02.pages.dev/", logo: "/customer-logos/aiport.png", brandIcon: "AI", brandClass: "aiport", description: "AIport managed customer environment.", sites: [
  ...
  { id: "CUS-004", name: "V. Regenerative", tier: "Managed", status: "Active", environments: 1, services: 1, website: "https://screenshot-exact-copy-23.lovable.app/", brandIcon: "V", brandClass: "regenerative", description: "V. Regenerative managed customer environment.", sites: [
...
const recentActivity = [
  ...
  { title: "Monitoring history enabled", detail: "Customer checks now persist locally with outage and recovery alerts", type: "ops" },
];
...
function App() {
  ...
  const [monitorHistory, setMonitorHistory] = useState(() => loadStored(HISTORY_KEY, {}));
  const [monitorAlerts, setMonitorAlerts] = useState(() => loadStored(ALERT_KEY, []));
  ...
  const monitoredCustomerCount = allCustomerSites.filter((site) => customerMonitor[site.slug]).length;
  const healthyCustomerCount = allCustomerSites.filter((site) => customerMonitor[site.slug]?.reachable).length;
  const customerWarnings = allCustomerSites.filter((site) => {
    const item = customerMonitor[site.slug];
    return item && (!item.reachable || (item.tls?.days_remaining != null && item.tls.days_remaining < 21) || (item.response_ms || 0) > 2500);
  }).length;
  const activeAlerts = monitorAlerts.filter((alert) => !alert.resolved).length;

  function emitBrowserAlert(title, body) {
    if (notificationState === "granted" && typeof Notification !== "undefined") {
      try { new Notification(title, { body }); } catch { /* browser policy may block */ }
    }
  }

  function addAlert(nextAlerts, alert) {
    return [{ id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, createdAt: new Date().toISOString(), resolved: false, ...alert }, ...nextAlerts].slice(0, ALERT_LIMIT);
  }

  function recordMonitorResults(nextMonitor) {
    const now = new Date().toISOString();
    setMonitorHistory((currentHistory) => {
      const nextHistory = { ...currentHistory };
      for (const site of allCustomerSites) {
        const item = nextMonitor[site.slug];
        if (!item) continue;
        const sample = { checkedAt: item.checked_at || now, reachable: Boolean(item.reachable), httpStatus: item.http_status ?? null, responseMs: item.response_ms ?? null, tlsDays: item.tls?.days_remaining ?? null };
        nextHistory[site.slug] = [...(nextHistory[site.slug] || []), sample].slice(-HISTORY_LIMIT);
      }
      saveStored(HISTORY_KEY, nextHistory);
      return nextHistory;
    });

    setMonitorAlerts((currentAlerts) => {
      let nextAlerts = [...currentAlerts];
      for (const site of allCustomerSites) {
        const item = nextMonitor[site.slug];
        if (!item) continue;
        const prior = monitorHistory[site.slug]?.[monitorHistory[site.slug].length - 1];
        const openForSite = nextAlerts.find((alert) => alert.slug === site.slug && !alert.resolved);
        const critical = !item.reachable;
        const warning = item.reachable && ((item.tls?.days_remaining != null && item.tls.days_remaining < 21) || (item.response_ms || 0) > 2500);

        if (critical && prior?.reachable !== false && !openForSite) {
          nextAlerts = addAlert(nextAlerts, { slug: site.slug, customerName: site.customerName, serviceName: site.name, severity: "critical", message: `Endpoint unavailable${item.http_status ? ` · HTTP ${item.http_status}` : ""}` });
          emitBrowserAlert(`Atlaris outage: ${site.customerName}`, `${site.name} is unavailable.`);
        } else if (warning && !openForSite) {
          const reason = (item.tls?.days_remaining != null && item.tls.days_remaining < 21) ? `TLS expires in ${item.tls.days_remaining} days` : `Latency ${item.response_ms} ms`;
          nextAlerts = addAlert(nextAlerts, { slug: site.slug, customerName: site.customerName, serviceName: site.name, severity: "warning", message: reason });
          emitBrowserAlert(`Atlaris warning: ${site.customerName}`, reason);
        } else if (!critical && !warning && openForSite) {
          nextAlerts = nextAlerts.map((alert) => alert.id === openForSite.id ? { ...alert, resolved: true, resolvedAt: now } : alert);
          nextAlerts = addAlert(nextAlerts, { slug: site.slug, customerName: site.customerName, serviceName: site.name, severity: "recovery", resolved: true, message: "Endpoint returned to healthy state" });
          emitBrowserAlert(`Atlaris recovery: ${site.customerName}`, `${site.name} is healthy again.`);
        }
      }
      nextAlerts = nextAlerts.slice(0, ALERT_LIMIT);
  ...
  async function refreshLiveMonitoring({ manual = false } = {}) {
    ...
    } catch (error) { if (manual) setOperation({ busy: false, message: `Live monitoring failed: ${error.message}`, tone: "error" }); }
  ...
  async function refreshCustomerMonitoring({ manual = false } = {}) {
    ...
    if (manual) setOperation({ busy: true, message: "Refreshing customer monitoring…", tone: "neutral" });
    try {
      const results = await Promise.allSettled(allCustomerSites.map((site) => apiFetch(`/api/ops/customer/monitor?customer=${encodeURIComponent(site.slug)}`)));
      const next = {};
      results.forEach((result, index) => {
        const site = allCustomerSites[index];
        next[site.slug] = result.status === "fulfilled" ? result.value : { customer: site.slug, reachable: false, error: result.reason?.message || "Monitor unavailable", checked_at: new Date().toISOString() };
      });
      setCustomerMonitor(next);
      recordMonitorResults(next);
      setCustomerMonitorUpdated(new Date());
      if (manual) setOperation({ busy: false, message: "Customer monitoring refreshed.", tone: "success" });
    } catch (error) { if (manual) setOperation({ busy: false, message: `Customer monitoring failed: ${error.message}`, tone: "error" }); }
  ...
  function LatencyBars({ slug }) {
    ...
    if (!history.length) return <div className="history-empty">Waiting for history samples…</div>;
  ...
  function renderMonitoring() {
    return <><SectionTitle eyebrow="Monitoring" title="Operational monitoring" description="Automatic service polling, customer health history and active alert visibility." action={<button className="primary-button" onClick={() => { refreshLiveMonitoring({ manual: true }); refreshCustomerMonitoring(); }} disabled={monitoringBusy || customerMonitorBusy}>Refresh now</button>} /><section className="stats-grid"><StatCard label="Infrastructure services" value={`${activeServiceCount}/${serviceDefinitions.length}`} detail="Approved runtime services active" accent="LIVE" /><StatCard label="Customer endpoints" value={`${healthyCustomerCount}/${allCustomerSites.length}`} detail="Production sites reachable" accent="WEB" /><StatCard label="Active alerts" value={activeAlerts} detail={customerWarnings ? `${customerWarnings} live warning conditions` : "No live warning conditions"} accent="ALERT" /><StatCard label="History samples" value={Object.values(monitorHistory).reduce((total, list) => total + list.length, 0)} detail="Persisted in this browser" accent="HIST" /></section><section className="overview-grid"><article className="panel"><SectionTitle eyebrow="Runtime" title="Live service summary" /><div className="service-list">{serviceDefinitions.map((service) => <div className="service-line" key={service.id}><span><StatusDot tone={serviceState[service.id] === "active" ? "green" : "blue"} />{service.label}</span><strong>{serviceState[service.id] || "Checking"}</strong></div>)}</div></article><article className="panel"><SectionTitle eyebrow="Customers" title="Customer service summary" /><div className="service-list">{allCustomerSites.map((site) => <div className="service-line" key={site.slug}><span><StatusDot tone={customerMonitor[site.slug]?.reachable ? "green" : "blue"} />{site.customerName} · {site.name}</span><strong>{customerMonitor[site.slug]?.reachable ? `${customerMonitor[site.slug].response_ms} ms` : customerMonitor[site.slug] ? "Review" : "Checking"}</strong></div>)}</div><button className="secondary-button monitor-button" onClick={() => setSection("customer-services")}>Open customer monitoring</button></article></section></>;
  }
