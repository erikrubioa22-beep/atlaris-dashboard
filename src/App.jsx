...
import './App.css'
import {buildIncidents,monitoringState} from './dashboardPolicy.js'
import InfrastructureCosts from './InfrastructureCosts.jsx'

const NativeAutomation=lazy(()=>import('./NativeAutomation.jsx'))
const NativeSettings=lazy(()=>import('./NativeSettings.jsx'))
...
const serviceDefinitions=[
 ...
 {id:'atlaris-public-ai',label:'Public AI Backend',detail:'Atlaris public AI service',restartable:true},
 {id:'atlaris-monitor',label:'Monitoring Engine',detail:'24/7 service monitor',restartable:true},
 {id:'atlaris-travel-api',label:'Travel API',detail:'Atlaris Travel application API',restartable:true},
 {id:'atlaris-finance-api',label:'Finance API',detail:'Finance and administration API',restartable:true},
]
const navGroups=[
 {label:'Workspace',items:[{id:'overview',label:'Overview',icon:'◫'}]},
 {label:'Infrastructure',items:[{id:'infrastructure',label:'Infrastructure',icon:'▤'},{id:'monitoring',label:'Monitoring',icon:'⌁'},{id:'infrastructure-costs',label:'Infrastructure Costs',icon:'$'}]},
 {label:'Customers',items:[{id:'customers',label:'Customer Directory',icon:'◎'},{id:'customer-services',label:'Customer Services',icon:'◇'}]},
 {label:'Administration',items:[{id:'email-accounts',label:'Email Accounts',icon:'✉'},{id:'automation',label:'Automation',icon:'⌘'},{id:'settings',label:'Settings',icon:'⚙'},{id:'system-health',label:'System Health',icon:'◉'}]},
]
...
async function api(path,options={}){const r=await fetch(path,{cache:'no-store',credentials:'same-origin',...options,headers:{...(options.body?{'Content-Type':'application/json'}:{}),...(options.headers||{})}});if(r.status===401){window.location.assign(`/login?next=${encodeURIComponent(location.pathname+location.search)}`);throw new Error('Session expired')}const t=await r.text();let d={};try{d=t?JSON.parse(t):{}}catch{d={detail:t}}if(!r.ok)throw new Error(typeof d.detail==='string'?d.detail:d.detail?.message||`HTTP ${r.status}`);return d}
...
function CustomerLogo({customer,large=false}){return <div className={`customer-monogram customer-logo ${large?'large':''}`}><span className="customer-logo-fallback" style={{display:'grid'}}>{customer.brandIcon||customer.name.slice(0,2).toUpperCase()}</span></div>}
function LoadingModule(){return <section className="panel"><div className="empty-state">Loading module…</div></section>}
function ageLabel(value){if(!value)return'No sample';const ms=Date.now()-new Date(value).getTime();if(ms<60000)return`${Math.max(1,Math.round(ms/1000))}s ago`;if(ms<3600000)return`${Math.round(ms/60000)}m ago`;return`${Math.round(ms/3600000)}h ago`}
...
export default function App(){
 ...
 const siteBySlug=useMemo(()=>Object.fromEntries(allSites.map(s=>[s.slug,s])),[allSites])
 const managedEvents=useMemo(()=>events.filter(e=>siteBySlug[e.slug]),[events,siteBySlug])
 const incidents=useMemo(()=>buildIncidents(managedEvents),[managedEvents])
 const serviceRows=useMemo(()=>allSites.map(site=>{const target=targetBySlug[site.slug];return{site,target,state:monitoringState(target||{})}}),[allSites,targetBySlug])
 const counts=useMemo(()=>({healthy:serviceRows.filter(x=>x.state.tone==='healthy').length,degraded:serviceRows.filter(x=>x.state.tone==='degraded').length,critical:serviceRows.filter(x=>x.state.tone==='critical').length,stale:serviceRows.filter(x=>x.state.tone==='stale').length}),[serviceRows])
 const activeServiceCount=serviceDefinitions.filter(s=>serviceState[s.id]==='active').length
 const allServicesHealthy=activeServiceCount===serviceDefinitions.length

 async function withOperation(label,fn){setOperation({busy:true,message:`${label}…`,tone:'neutral'});try{const x=await fn();setOperation({busy:false,message:`${label} completed successfully.`,tone:'success'});return x}catch(e){setOperation({busy:false,message:`${label} failed: ${e.message}`,tone:'error'});throw e}}
 ...
 async function createBackup(slug){await withOperation(`Creating ${slug} backup`,()=>api(`/api/ops/customer/backup?customer=${encodeURIComponent(slug)}`,{method:'POST'}));await loadBackups(slug)}
 async function restoreBackup(slug,name){if(!window.confirm(`Restore ${slug} from ${name}? A safety backup is created first.`))return;await withOperation(`Restoring ${slug}`,()=>api('/api/ops/customer/restore',{method:'POST',body:JSON.stringify({customer:slug,backup:name})}));await loadBackups(slug)}
 async function loadLogs(id){const d=await withOperation(`Loading ${id} logs`,()=>api(`/api/ops/service/logs?service=${encodeURIComponent(id)}&lines=80`));setSelectedLogService(id);setServiceLogs(d.content||'No log entries returned.')}
 async function restartService(id){if(!window.confirm(`Restart ${id}?`))return;const d=await withOperation(`Restarting ${id}`,()=>api('/api/ops/service/restart',{method:'POST',body:JSON.stringify({service:id})}));setServiceState(x=>({...x,[id]:d.active||'active'}))}
 ...
 function renderEmail(){return <><SectionTitle eyebrow="Administration" title="Email Accounts" description="Atlaris-managed mailboxes."/><section className="panel"><div className="service-list">{emailAccounts.map(x=><div className="service-line" key={x}><span><StatusDot/>{x}</span><strong>Active</strong></div>)}</div></section></>}
 function renderSystemHealth(){const capacity='atlas01 has substantial CPU, memory and disk headroom; atlas02 is a resilience/DR objective rather than a capacity requirement.';return <><SectionTitle eyebrow="Resilience" title="System Health & Zero Touch Readiness" description="Operational posture, backup readiness and atlas02 decision framework." action={<button className="secondary-button" onClick={loadSystemHealth}>Refresh</button>}/><section className="stats-grid"><StatCard label="Runtime services" value={`${activeServiceCount}/${serviceDefinitions.length}`} detail="Approved atlas01 services" accent="SYS"/><StatCard label="Finance API" value={financeHealth?.status||'Checking'} detail="Administration data plane" accent="DB"/><StatCard label="Finance backups" value={financeBackup?.count??'—'} detail={financeBackup?.latest?`Latest ${new Date(financeBackup.latest.modified_at).toLocaleString()}`:'Controlled backup available'} accent="BKP"/><StatCard label="atlas02" value="Standby later" detail="DR / HA, not performance scaling" accent="DR"/></section><section className="overview-grid"><article className="panel"><SectionTitle eyebrow="Zero Touch resilience" title="Current protection"/><div className="safety-list">{['Atomic Dashboard releases with rollback','Nginx validation before release activation','Finance PostgreSQL controlled backups','Customer source backups and safety backups before restore','24/7 monitoring with incident lifecycle','Unified customer registry stored in customer metadata'].map(x=><div key={x}><span>✓</span>{x}</div>)}</div></article><article className="panel"><SectionTitle eyebrow="atlas02" title="Standby architecture"/><p>{capacity}</p><div className="safety-list">{['Replicate configuration and customer metadata','Maintain standby Nginx and control-plane packages','Replicate/ship Finance backups off atlas01','Health-check atlas01 from atlas02','Document DNS/ingress failover','Promote only after automated restore drills pass'].map(x=><div key={x}><span>→</span>{x}</div>)}</div></article></section></>}

 const nativeSections={overview:renderOverview,infrastructure:renderInfrastructure,monitoring:renderMonitoring,'infrastructure-costs':()=> <InfrastructureCosts/>,customers:renderCustomers,'customer-services':renderCustomerServices,'email-accounts':renderEmail,'system-health':renderSystemHealth}
 return <div className="app-shell"><aside className="sidebar"><div className="brand-block"><div className="brand-mark">A</div><div><div className="brand-name">Atlaris</div><div className="brand-caption">Administration Platform</div></div></div><div className="nav-scroll">{navGroups.map(g=><div className="nav-group" key={g.label}><div className="sidebar-section-label">{g.label}</div><nav className="nav-list" aria-label={g.label}>{g.items.map(i=><button key={i.id} className={`nav-link ${section===i.id?'active':''}`} onClick={()=>setSection(i.id)}><span>{i.icon}</span>{i.label}</button>)}</nav></div>)}</div><div className="sidebar-health"><div className="health-line"><span><StatusDot tone={allServicesHealthy?'green':'blue'}/>Atlaris platform</span><strong>{allServicesHealthy?'Online':`${activeServiceCount}/${serviceDefinitions.length}`}</strong></div><div className="health-line"><span><StatusDot tone={counts.critical===0?'green':'blue'}/>Customer sites</span><strong>{counts.healthy}/{allSites.length}</strong></div><div className="health-line muted"><span>Open incidents</span><strong>{incidents.active.length}</strong></div></div></aside><main className="main-content"><header className="topbar"><div><span className="eyebrow">Atlaris Technologies · Administration</span><h1>{section==='overview'?'Administration overview':sectionLabel}</h1><p>Secure infrastructure and customer operations from one control plane.</p></div><div className="top-actions"><span className="live-pill"><StatusDot tone={incidents.active.length===0&&allServicesHealthy?'green':'blue'}/>{incidents.active.length?`${incidents.active.length} open incident${incidents.active.length===1?'':'s'}`:'24/7 monitoring healthy'}</span><div className="user-chip">AR</div><a className="secondary-button logout-button" href="/logout">Logout</a></div></header>{nativeSections[section]?.()}{section==='automation'&&<Suspense fallback={<LoadingModule/>}><NativeAutomation/></Suspense>}{section==='settings'&&<Suspense fallback={<LoadingModule/>}><NativeSettings/></Suspense>}<footer className="footer-note"><span>Atlaris Administration Platform</span><span>Unified registry · on-demand history · shared health policy · incident lifecycle · controlled recovery</span></footer></main><style>{`.monitor-badge.degraded,.monitor-badge.stale{background:#332714;color:#e4bd70}.monitor-badge.critical{background:#32161d;color:#ef97a4}.monitor-badge.healthy{background:#102a23;color:#7bd3b4}.customer-monitor-row.degraded,.customer-monitor-row.stale{background:rgba(71,56,23,.16)}.customer-monitor-row.critical{background:rgba(82,28,37,.15)}`}</style></div>
}
