...
const recentActivity = [
  ...
  { title: "Customer monitoring enabled", detail: "HTTP, latency and TLS checks available for all managed sites", type: "ops" },
];
...
function App() {
  ...
  async function refreshCustomerMonitoring({ manual = false } = {}) {
    ...
    try {
      ...
      results.forEach((result, index) => {
        ...
        next[site.slug] = result.status === "fulfilled" ? result.value : { customer: site.slug, reachable: false, error: result.reason?.message || "Monitor unavailable" };
      });
  ...
  function renderOverview() {
    ...
    return <>
      <section className="hero-grid">
        ...
        <article className="hero-card customers-hero" onClick={() => setSection("customer-services")}><div className="hero-icon">◎</div><div><span className="eyebrow">Customers</span><h3>Monitor customer services</h3><p>Live availability, HTTP status, response time, TLS certificate health, backups and recovery operations.</p></div><span className="hero-arrow">→</span></article>
      </section>
      <section className="stats-grid"><StatCard label="Live services" value={serviceSummary} detail={monitoringLastUpdated ? `Updated ${monitoringLastUpdated.toLocaleTimeString()}` : "Automatic monitoring starting"} accent="LIVE" /><StatCard label="Customer sites" value={customerSummary} detail={customerWarnings ? `${customerWarnings} warning${customerWarnings === 1 ? "" : "s"} detected` : "No customer warnings"} accent="WEB" /><StatCard label="Managed customers" value={customers.length} detail={`${allCustomerSites.length} monitored endpoints`} accent="CRM" /><StatCard label="MCP capabilities" value="40" detail="Controlled administration tools" accent="MCP" /></section>
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand-block"><div className="brand-mark">A</div><div><div className="brand-name">Atlaris</div><div className="brand-caption">Administration Platform</div></div></div><div className="nav-scroll">{navGroups.map((group) => <div className="nav-group" key={group.label}><div className="sidebar-section-label">{group.label}</div><nav className="nav-list" aria-label={group.label}>{group.items.map((item) => <button key={item.id} className={`nav-link ${section === item.id ? "active" : ""}`} onClick={() => setSection(item.id)}><span>{item.icon}</span>{item.label}</button>)}</nav></div>)}</div><div className="sidebar-health"><div className="health-line"><span><StatusDot tone={allServicesHealthy ? "green" : "blue"} />Atlaris platform</span><strong>{allServicesHealthy ? "Online" : checkedServiceCount ? `${activeServiceCount}/${serviceDefinitions.length}` : "Checking"}</strong></div><div className="health-line"><span><StatusDot tone={healthyCustomerCount === allCustomerSites.length ? "green" : "blue"} />Customer sites</span><strong>{monitoredCustomerCount ? `${healthyCustomerCount}/${allCustomerSites.length}` : "Checking"}</strong></div><div className="health-line muted"><span>MCP tools</span><strong>40</strong></div></div></aside><main className="main-content"><header className="topbar"><div><span className="eyebrow">Atlaris Technologies · Administration</span><h1>{section === "overview" ? "Administration overview" : sectionLabel}</h1><p>Secure infrastructure and customer operations from one control plane.</p></div><div className="top-actions"><span className="live-pill"><StatusDot tone={allServicesHealthy && healthyCustomerCount === allCustomerSites.length ? "green" : "blue"} />{customerWarnings ? `${customerWarnings} monitoring warning${customerWarnings === 1 ? "" : "s"}` : "Live monitoring healthy"}</span><div className="user-chip" title="Atlaris administrator">AR</div><a className="secondary-button logout-button" href="/logout">Logout</a></div></header>{section === "overview" && renderOverview()}{section === "infrastructure" && renderInfrastructure()}{section === "monitoring" && renderMonitoring()}{section === "customers" && renderCustomers()}{section === "customer-services" && renderCustomerServices()}{section === "email-accounts" && renderEmailAccounts()}{!["overview", "infrastructure", "monitoring", "customers", "customer-services", "email-accounts"].includes(section) && renderPlaceholder()}<footer className="footer-note"><span>Atlaris Administration Platform</span><span>Session authenticated · infrastructure monitoring · customer monitoring · controlled recovery</span></footer></main></div>;
}
